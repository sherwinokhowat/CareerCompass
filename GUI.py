...
class CareerCompass:
    ...
    def __init__(self, root):
        ...
        self.images = {
            ...
            'find_jobs': ImageTk.PhotoImage(Image.open(folder / 'assets' / 'FindJobsButton.png')),
            ...
            'URLButton': ImageTk.PhotoImage(Image.open(folder / 'assets' / 'URLButton.png'))
        }
...
class HomePage(ttk.Frame):
    ...
    def __init__(self, container_home, app):
        ...
        # Background
        canvas = tk.Canvas(self, bg="#0E355D", height=750, width=1000, bd=0, highlightthickness=0,
                           relief="ridge")
        canvas.create_rectangle(0, 0, 270.0, 750.0, fill="#0084FF", outline="")
        canvas.create_image(138.0, 154.0, image=self.app.images['line'])

        # Title
        canvas.create_text(7.0, 4.0, anchor="nw", text="Career", fill="#0E355D", font=("Karma Bold", 64 * -1))
        canvas.create_image(63.0, 107.0, image=self.app.images['compass_emoji'])
        canvas.create_text(7.0, 58.0, anchor="nw", text="C  mpass", fill="#0E355D", font=("Karma Bold", 64 * -1))
        ...
        preferences_button = tk.Button(self, image=self.app.images['pbutton'], borderwidth=0, highlightthickness=0,
                                       command=self.start_button_command, relief="flat")
        ...
        contact_button = tk.Button(self, image=self.app.images['cbutton'], borderwidth=0, highlightthickness=0,
                                   command=lambda u=contact_url: webbrowser.open(u), relief="flat")
        ...
        # Title
        canvas.create_text(625.0, 75.0, anchor="center", text="Launch Your Career", fill="#D6F4FF",
                           font=("Karma Bold", 64 * -1))

        # Input Name
        canvas.create_text(383.0, 155.0, anchor="nw", text="What's your name?", fill="#FFFFFF",
        ...
        canvas.create_text(465.0, 530.0, anchor="center", text="Jobs Available!", fill="#0E355D",
                           font=("Karma Medium", 36 * -1))
        canvas.create_text(465.0, 470.0, anchor="center", text=self.app.num_jobs, fill="#FFFFFF",
                           font=("Karma Bold", 64 * -1))

        # Average Pay Fact
        canvas.create_image(800.0, 500.0, image=self.app.images['gradient_background'])
        canvas.create_text(800.0, 530.0, anchor="center", text="Average Pay", fill="#0E355D",
                           font=("Karma Medium", 36 * -1))
        canvas.create_text(800.0, 470.0, anchor="center", text="$99,999", fill="#FFFFFF",
                           font=("Karma Bold", 64 * -1))
...
class PreferencesPage(ttk.Frame):
    ...
    def __init__(self, container_preferences, app):
        ...
        # Title
        canvas.create_text(7.0, 4.0, anchor="nw", text="Career", fill="#0E355D",
                           font=("Karma Bold", 64 * -1))
        canvas.create_image(63.0, 107.0, image=self.app.images['compass_emoji'])
        canvas.create_text(7.0, 58.0, anchor="nw", text="C  mpass", fill="#0E355D",
                           font=("Karma Bold", 64 * -1))
        ...
        # Page Title
        canvas.create_text(625.0, 70.0, anchor="center", text=f"{self.app.user_name}, "f"let's see what you prefer.",
        ...
        for i in range(7):
            canvas.create_text(385.0, initial_y - 34, anchor="nw", text=questions[i], fill="#FFFFFF",
        ...
        # Next Button
        find_jobs_button = tk.Button(self, image=self.app.images['find_jobs'], borderwidth=0, highlightthickness=0,
                                     command=lambda: self.find_jobs_button_command(), relief="flat")
...
class JobsPage(ttk.Frame):
    ...
    def __init__(self, container_jobs, app):
        ...
        # Background
        canvas = tk.Canvas(self, bg="#0E355D", height=750, width=1000, bd=0, highlightthickness=0, relief="ridge")
        canvas.create_rectangle(0, 0, 270.0, 750, fill="#0084FF", outline="")
        ...
        # Page Text
        canvas.create_text(625.0, 70.0, anchor="center", text=f"{self.app.user_name}, find your job!",
                           fill="#D6F4FF", font=("Karma Bold", 40 * -1))
        canvas.create_text(414.0, 98.0, anchor="nw", text="Click one of the stars to find similar jobs!",
        ...
        for job in self.app.job_postings:
            ...
            if job.job_details['city'] == '':
                ...
                location = (job.job_details['city'] + ', ' + job.job_details['country'])[:50]
            ...
            star = tk.Button(self, image=self.app.images['star_img'], borderwidth=0, highlightthickness=0,
                             command=lambda temp=job: self.star_button_command(temp), relief="flat")
            star.place(x=322.0, y=initial_y, width=44.0, height=44.0)
            url_button = tk.Button(self, image=self.app.images['URLButton'], borderwidth=0, highlightthickness=0,
                                   command=lambda temp=job_url: self.open_url(temp), relief="flat")
            url_button.place(x=900.0, y=initial_y - 20, width=15.0, height=83.0)
            canvas.create_image(635.0, initial_y + 19, image=self.app.images['job_posting_img'])
            canvas.create_text(635.5, initial_y - 15, anchor="center", text=title, fill="#FFFFFF",
                               font=("Karma Bold", 20 * -1))
            canvas.create_text(635.0, initial_y + 8, anchor="center", text=details, fill="#0E355D",
                               font=("Karma Bold", 18 * -1))
            canvas.create_text(635.0, initial_y + 40, anchor="center", text=desc,
                               fill="#FFFFFF", font=("Karma Light", 15 * -1))
